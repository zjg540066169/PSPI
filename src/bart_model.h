...
// 
// [[Rcpp::export]]
SEXP bart_logit(NumericMatrix X, NumericVector Y, long nburn = 100, long npost = 1000, bool verbose = true){
  
  NumericVector Y_ = clone(Y);
  NumericVector Z = clone(Y) - 0.5;
  bart_model * m = new bart_model(X, Z);
  List a;
  NumericVector w = 1 + NumericVector(Y.length());
  a = m -> update(1, w, 1, 1, 1, verbose, 100);
  NumericMatrix y_pre(1000, Y.length());
  for(int j = 0 ; j < nburn + npost ; ++j){
    Rcout << j << " " << nburn + npost << std::endl;
    NumericVector bart_pre = colMeans(m->predict(X));
    for(int i = 0 ; i < Y.length(); ++i){
      w[i] = pg::rpg_scalar_hybrid(1, bart_pre[i]);
      Z[i] = bart_pre[i] + ((double)Y_[i] - 0.5) / w[i];
      y_pre(j, i) = R::rbinom(1, R::plogis(bart_pre[i], 0, 1, true, false));
    }
    Rcout << w << std::endl;
    m->set_data(X, Z);
    m->update(1, sqrt(w), 1, 1, 1, false, 10L);
  }
  
  
  return List::create(Named("y_pre") = y_pre);
  //return List();
